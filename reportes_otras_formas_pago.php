...
$query_total = "SELECT COUNT(*) as total 
                ...
                WHERE (r.monto_otra_forma_pago > 0 OR r.total_con_impuesto > 0)
                AND f.tipo != 'efectivo'  -- Excluir pagos en efectivo
...
$query = "SELECT r.numero_factura, f.banco, f.cuenta, r.total_con_impuesto, r.monto_otra_forma_pago 
          FROM reportes r
          LEFT JOIN formas_pago f ON r.forma_pago_id = f.id
          WHERE (r.monto_otra_forma_pago > 0 OR r.total_con_impuesto > 0)
          AND f.tipo != 'efectivo'  -- Excluir pagos en efectivo
...
while ($row = $result->fetch_assoc()) {
    ...
    // Verificar si los campos 'banco' y 'cuenta' están vacíos o no existen
    $row['banco'] = isset($row['banco']) && !empty($row['banco']) ? $row['banco'] : 'N/A';
    $row['cuenta'] = isset($row['cuenta']) && !empty($row['cuenta']) ? $row['cuenta'] : 'N/A';

    $reportes[] = $row;
}

// Consulta para obtener el acumulado de los montos de "Valor Otra Forma de Pago" que coinciden con los filtros aplicados
$query_acumulado = "SELECT SUM(
                        CASE 
                            WHEN r.monto_otra_forma_pago > 0 THEN r.monto_otra_forma_pago
...
// Consulta para obtener el total por mes de los montos de "Valor Otra Forma de Pago"
...
// Consulta para obtener el total por día de los montos de "Valor Otra Forma de Pago"
...
<head>
    ...
    <title>Reporte de Otras Formas de Pago</title>
    <!-- Bootstrap CSS -->
...
<body>
    <div class="container">
        <h1>Reporte de Otras Formas de Pago</h1>
        ...
        <div class="mb-3">
            <a href="reportes_otras_formas_pago.php" class="btn btn-warning">Limpiar Filtros</a>
        </div>
            ...
            <tbody>
                ...
                <?php else: ?>
                    <?php foreach ($reportes as $reporte): ?>
                        <?php
                        // Calcular el valor de otra forma de pago
                        $valorOtraFormaPago = $reporte['monto_otra_forma_pago'] > 0 
                            ? $reporte['monto_otra_forma_pago'] 
                            : $reporte['total_con_impuesto'];
                        ?>
                        <tr>
                            <td><?php echo htmlspecialchars($reporte['numero_factura']); ?></td>
                            <td><?php echo htmlspecialchars($reporte['banco']); ?></td>
                            <td><?php echo htmlspecialchars($reporte['cuenta']); ?></td>
                            <td><?php echo number_format($reporte['total_con_impuesto'], 2); ?></td>
                            <td><?php echo number_format($valorOtraFormaPago, 2); ?></td>
                        </tr>
                    <?php endforeach; ?>
                <?php endif; ?>
            </tbody>
